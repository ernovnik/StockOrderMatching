...
class Repository {
  ...
  def getClients = {
    Source.fromInputStream(getClass.getClassLoader.getResourceAsStream("clients.txt")).getLines.map(
      line=> {
        line match{
  ...
  def getOrders = {
    io.Source.fromInputStream(getClass.getClassLoader.getResourceAsStream("orders.txt")).getLines.map(
        line=>{
          line match {
